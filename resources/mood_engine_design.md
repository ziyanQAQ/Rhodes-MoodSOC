...
| ~~P5b-1~~ | ~~`M17` 元修正（摩根 头号陪练）~~ ✅ 见 §P5b | 目标 ②③ |
| ~~P5b-2~~ | ~~`M15a` 进驻事件（患难之交心情互换）~~ ✅ 见 §P5b2 | 目标 ②③ |
| ~~P5c~~ | ~~训练室 9 条技能（设施纳入每小时模型）~~ ✅ | 目标 ① |
| **P5-b** | `M15b` 触发式恢复、加工站按次消耗（上游缺字段） | 目标 ①② |
...
见 `skill_taxonomy.md` §5.7。要点：**摩根自己不回复，而是把推进之王那条贡献顶上去**。
数据用 `boost_provider`（强化谁）+ `boost_group`（强化哪一组）+ `CLAUSE_COND` 目标筛选；
实现把增量记成**同组同 skill_id** 的额外贡献，交给 `SAME_KIND_MAX` 并入被强化技能的小计。

> 这是本项目**第一次建模"技能改技能"**（轴 A 里原本归为"登记不建模"的那一类）。
> 它没有新增任何轴 F 分支——复用 `SAME_KIND_MAX` 即可，这反过来验证了 P2 的记录层抽象够用。

顺带修掉流水账 `explain()` 的一处误标：同技能多分句时，它拿**单条分句**的 value 去和
**技能小计**比，导致获胜技能的每条分句都被标成"被更高者覆盖"（实测 0.2 与 0.3 都被标）。

### P5b2 —— `M15a` 进驻事件（已完成）

见 `skill_taxonomy.md` §5.8。这一条**迫使模型承认"事件 ≠ 速率"**：

- 患难之交（菲亚梅塔）的效果是**进驻那一刻**的心情互换，不是每小时 ±N。
  正确做法不是往 `rules.py` 的速率计算里塞分支，而是**另开一个语义层**：
  `rules.apply_entry_events(world)`（就地改心情，CLI `--entry-events`），
  事件本身记 `Bucket.EVENT` 供展示。
- 「前一位进驻」不需要新数据结构——`Facility.operators` 本来就是**有序列表**（进驻顺序）。
  这是 P1 把布局做成一等公民时留下的红利。
- `evaluate` / `evaluate_base` **不**自动调用：函数名承诺"只读测算"。

### P5c —— 训练室纳入每小时模型（已完成）

目标 ① 里"缺失设施"的最后一个。两处独立佐证把口径钉死：

1. 需求文档 `docx` **第 4 段**：「干员工作时，在无额外心情加减的情况下，每小时的**基础消耗速率为 1 点心情**」
   ——不区分设施，所以 `TRAINING_BASE_CONSUMPTION = 1` 是**文档写明的**，不再是"按增量语义猜的"。
2. 上游 `building_data.json → buffs`：**9 条**训练室 buff 原文均为「…时，心情每小时消耗 **+1**」。

改动：`FACILITY_BY_PREFIX` 加 `"train" → TRAINING`；`MODELED_ROOMS` 加 `TRAINING`；
9 条以 `SELF_CONSUME +1`（`M07a`）进 `moods_skills.txt`。台账里这 9 条从
`TRAINING 未纳入每小时模型（设施未建模）` 变为 `clause 级已建模`。

**顺带查清加工站**（同一轮的 §11 查证产物）：上游加工站提到「心情」的 buff **35 条**，
原文**全部**是「**配方**心情消耗」（-1/-2/-4、恒定、除以 4、+2），印证加工站**没有**每小时消耗。
但**配方心情消耗在上游 dump 里没有字段**（`workshopFormulas` 68 条只有
`apCost/goldCost/costs/…`，递归搜 `mood` 零命中），所以"每次加工扣多少心情"**无法落库**，
35 条 `X08` 与棘刺「爆炸艺术」保持登记不建模。

> 这一轮的价值不在"多算了 9 条技能"，而在**把"猜"换成"查"**：
> §4.14/§4.15 两条从"待确认/按语义取"升级为"文档 + 上游双证据"，并明确写下
> **哪些数据上游根本没有**（免得下次又有人去 `apCost` 上反推）。
...
| “爆炸艺术”（棘刺） | 「每 **2 次**加工没有产出副产品则恢复自身一次心情」——需要"加工次数"这一环境事实，布局里没有 |
