...
- L3 描述对照：✅ 245 条一致 / ⚪ 4 条无数字可对（消除/事件类）/ ⚪ 1 条已登记差异 / ❌ 0 条不符

### 需要处理的差异

没有。250 条 clause 都能在**满足它条件的场景**里按模板口径生效，上游描述里的数字与数据表一致，755 条 buff 也全部有归宿。
- ⚠️ **control_mp_cost&bd1_000#1**（已知差异，待拍板）：夕「不以物喜」上游写「控制中枢内所有干员的心情每小时恢复+0.05」，数据表按 M05「同设施全体消耗 -0.05」建模；两者净效果相同，但消耗侧归零钳位时会有 0.05 的差别，且与模板归属（M03 vs M05）不一致——待拍板
...
| M03 | 中枢内 全体回复 | A1 | F1 | 24 |
| M04 | 中枢内 指定干员共事 | A1 | F1 | 2 |
| M05 | 同设施 全体消耗增减 | A1 | F1 | 9 |
...
| `control_mp_cost&bd1_000#1` | "不以物喜" | facility_consume | M05 | -0.05 | ✅ ok | 同设施全体消耗 → -0.05（2 条实例：同设施全体（含自身）） | ⚪ |
...
| `control_mp_cost&bd1[000]` | "不以物喜" | CONTROL | A1 | M05 | yes | 1 | ✅ 全部核对 |
...
- `control_mp_cost&bd1_000#1`：夕「不以物喜」上游写「控制中枢内所有干员的心情每小时恢复+0.05」，数据表按 M05「同设施全体消耗 -0.05」建模；两者净效果相同，但消耗侧归零钳位时会有 0.05 的差别，且与模板归属（M03 vs M05）不一致——待拍板
