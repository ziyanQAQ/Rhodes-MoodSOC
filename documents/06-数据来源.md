...
> 覆盖 **§11 数据查找策略**（原 `resources/data_lookup_policy.md`）。
> 遇到任何「不知道的数据」先读本文，不要猜。

---

> 本节原为 `AGENTS.md` §11，现独立成文。根目录 `AGENTS.md` 只保留策略要点与速查表，
> 细节（拉取命令、四个坑、查完之后的三件事、上游缺失清单）都在本文。
> **改了本文的规则，`AGENTS.md` 的摘要也要跟着改。**
...
1. 修正 `resources/moods_skills.txt` / `operators.txt`（或对应的上游派生物）
   ...
   → 跑技能全量核对 `.venv/Scripts/python.exe scripts/verify_skills.py --check`
     （逐条 clause 造场景核对 + 台账双向核对 + **上游描述数字对照**；找不到上游就跳过 L3，
     可用 `--agd <目标目录>` 或环境变量 `ARKNIGHTS_GAMEDATA` 指定）
   → 跑全量 unittest（`.venv/Scripts/python.exe -m unittest discover -s tests`）。
2. 若该数据**推翻了既有假设**，同步更新 `04-特殊机制.md`（建模假设）或
   `documents/05-技能分类大纲.md` §5「决策记录」，避免下次又被当成 bug 或又被猜一遍。
...
| **训练室的「心情消耗」常量** | **无** | 顶层 `trainingData` 只有 `{basicSpeedBuff: 0.05, phases:[{specSkillLvlLimit:…}]}`——全是训练**速度**。（训练室已被判定为「挂件位」不建模，这个缺失不影响结论。） |
