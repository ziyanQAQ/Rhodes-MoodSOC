...
消耗 = base_consumption(设施类型)   ← 见 config.BASE_CONSUMPTION_BY_FACILITY
     ...
     ± 设施级技能  facility_consume（黍 -0.1、**巫恋"低语"+0.25**；对同设施全体含自身生效）
...
**中枢内**（`control_center`）：`cc_recover` 里 `facility_types == (控制中枢,)` 的那些——
左膀右臂 / S.W.E.E.P. / 笑靥如春等 15 条各 +0.05（求和）、德才兼备等 6 条按阵营人数 ×0.05、
浮生得闲（与阿同中枢）+0.25、魔王传承（与阿米娅同中枢）+0.05、
**夕「不以物喜」+0.05**（上游原文是"心情每小时**恢复**+0.05"，故走回复侧，见
`05-技能分类大纲.md` §5.12）。

**宿舍内**（`rules._dorm_ledger`）：
...
- 叠加规则：**不同种类（自身/群体/单体/定向）之间相加；同一种类内部取最高**。
  例外：定向回复（dorm_targeted）内部是**求和**而非取最高（`_dorm_ledger` 内的定向段直接累加；
  `rules._targeted_recovery` 只是留给外部调用的取值包装）。

### 3.2.1 布局的规模与等级（上游硬约束）

布局**不是固定的**：制造站 / 贸易站 / 发电站的数量可以互换，但受上游两条约束
（`building_data.json`，客户端 2.7.71）：

| 约束 | 上游字段 | 数值 | 项目里的落点 |
|---|---|---|---|
| 单类型房间数 | `rooms[].maxCount` | 制造 **5** / 贸易 **5** / 发电 **3** / 宿舍 **4** / 活动室 **6** / 会客·办公·训练·加工 **1** | `facility_max_count()`、`BaseLayout.validate()` |
| **建造位总量** | `layouts.v0.slots` 里 `category="OUTPUT"` 的槽位 | **9 个**（制造/贸易/发电**共用**） | `OUTPUT_SLOT_TOTAL = 9`、`validate()` 报"超出建造位总量 9：列出是哪几间" |
| 各等级容量 | `rooms[].phases[lv].maxStationedNum` | 制造/贸易 **1/2/3**、发电 1/1/1、宿舍 5/5/5/5/5、**控制中枢 1/2/3/4/5**、会客 2/2/2、训练 2/2/2、加工·办公 1/1/1 | `FACILITY_SLOTS_BY_LEVEL`（与上游逐项一致）、`facility_slots()`、`min_level_for_slots()` |
| 中枢等级 → 全基建减免 | `controlData.basicCostBuff = -5`（**每人 −0.05**，无逐等级字段） | 等级只决定**能站几个人**：Lv5 = 5 人 → **−0.25/时** | `cc_reduction(中枢人数)` |
| 宿舍等级 → 基础回复 | 同上（宿舍等级表见 3.2） | Lv1~5 = 2.0/2.5/3.0/3.5/**4.0** 点/时 | `dormitory_recovery(level)` |

**等级从哪来**：MAA 排班文件**不带等级**，所以导入时按"实际放了几个人"反推**最低可行等级**
（`maa.min_level_for_slots`）：制造站放 3 人 → Lv3；控制中枢放 5 人 → **Lv5**
（默认值只有 1，不推断就会出现"Lv1 容量 1 却站 5 人"这种自相矛盾的布局）。
宿舍保持默认 Lv5——宿舍等级决定基础回复，不能往下压。导入后可以逐间改：
**点看板房间卡头**（`LevelDialog`）或**批量设置的「房间等级」区**。
