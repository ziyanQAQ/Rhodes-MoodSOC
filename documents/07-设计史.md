...
> ⚠️ **本节是重构前的蓝图，实施时做了合并**——读它时请对照下面这张表，不要去找不存在的模块：
>
> | 蓝图里的名字 | 实际落地在哪 |
> |---|---|
> | `layout.py`（新） | 并入 **`mood_soc/models.py`**（`Facility` / `BaseLayout` 的复数查询方法）+ `scenario.py` 解析 |
> | `effects.py`（新，handler 注册表） | **未单独成文件**：效果派发就是 `rules.consume_ledger` / `recovery_ledger` 里的按 `SkillKind` / `template_id` 循环 |
> | `Accumulator`（新） | 落成 **`mood_soc/ledger.py` 的 `MoodLedger.total()`**（按 `Stacking` 合并，轴 F 只有这一份实现） |
> | `variables.py`（新） | ✅ 已建，就是 `mood_soc/variables.py`（变量账本） |
> | `ledger.py`（新） | ✅ 已建，`Contribution` + `MoodLedger` + `same_kind_winner` + `explain()` |
>
> 结论：**五个抽象收敛成了两个新文件**（`ledger.py` / `variables.py`），P1/P2/P3 见下方"三、已完成"。
...
| ~~P5c~~ | ~~训练室 9 条技能~~ ✅ 已查清；**最终判定：挂件位，不建模** | 目标 ① |
| ~~P5d~~ | ~~投资·α/β 心情常驻（贸易站限定）~~ ✅ 见 `04-特殊机制.md` 第 32 条 | 目标 ② |
| **P5-b** | ~~`M15b` 触发式恢复、加工站按次消耗（上游缺字段）~~ → **已定处置：加工类，不处理** | — |
| ~~P5-c~~ | ~~`documents/05-技能分类大纲.md` 缺口清单更新~~ ✅ | 可维护性 |
...
> ⚠️ **后续口径变更（覆盖上表两行的效果）**：潮汐守望的「每有 1 个深海猎人」已改为
> **含持有者自身**（`05-技能分类大纲.md` §5.9 / `04-特殊机制.md` 第 23 条）。
> 于是 `_cond_no_abyssal_outside_dorm` 与 `_cond_dorm_abyssals_full_mood` 虽然都挂上了，
> 却**恒不成立**——两个分句保留入口与数据，但不产生回复。
...
2. **`MoodLedger.same_kind_winner(bucket, group)`**：`_single_recovery` 旧实现按**单条分句**取 max，
   ...
   > 判据（写进 `09-开发指南.md` 的自查清单）：凡是"同种取最高"的取值点，都不许自己写 `max()`。
...
**遗留简化**：`_single_recovery` 仍是"全局一名受益者"（`04-特殊机制.md` 建模假设 5），
故「毒剂师之友」上游未写「除自身以外」、深靛本可自指，本模型仍把她排除在候选外。

### P5b —— `M17` 元修正（已完成）

见 `documents/05-技能分类大纲.md` §5.7。要点：**摩根自己不回复，而是把推进之王那条贡献顶上去**。
...
见 `documents/05-技能分类大纲.md` §5.8。这一条**迫使模型承认"事件 ≠ 速率"**：
...
### P5c —— 训练室：先纳入、后按「挂件位」撤回

> ⚠️ **本节当时的结论已被用户口径推翻**：训练室与加工站都是「**挂件位**」，
> **不计算心情消耗**，那 9 条「心情每小时消耗 +1」不生效。
> 最终口径见 `04-特殊机制.md` 第 15 条。下面保留的是当时的推理与上游证据（证据仍有效）。

目标 ① 里"缺失设施"的最后一个。两处独立佐证把口径钉死：

1. 需求文档 `docx` **第 4 段**：「干员工作时，在无额外心情加减的情况下，每小时的**基础消耗速率为 1 点心情**」
   ——当时据此取 `TRAINING_BASE_CONSUMPTION = 1`；**后来用户指出挂件位不属于"上岗生产"，改为 0**。
...
**顺带查清加工站**（同一轮按 `06-数据来源.md` 查证的产物）：上游加工站提到「心情」的 buff **35 条**，
...
> `04-特殊机制.md` 第 14/15 条从"待确认/按语义取"升级为"文档 + 上游双证据"，并明确写下
...
| 子句 | 处置 |
|---|---|
| 投资·α / 投资·β（龙舌兰） | ✅ **已解决（P5d）**：上游原文「如果下笔赤金订单交付数大于 3」只修饰**龙门币收益**，心情 **-0.25 是常驻的**，且只在**贸易站**生效。原按 `partial_mode=hold` 整条挡掉（减耗丢失），现转 `apply` —— 见 `04-特殊机制.md` 第 32 条 / `05-技能分类大纲.md` §5.10。 |
| “爆炸艺术”（棘刺，`M15b`） | 「每 **2 次**加工没有产出副产品则恢复自身一次心情」——**属加工类技能，用户已明确不需要处理**：它需要"加工次数"这一环境事实，且与 `X08` 加工站「配方心情消耗」同类（上游 dump 无该字段）。 |
