...
│   ├── batch.py           「干员与心情」内容本体：当前布局的干员 + 练度 + 心情一张表改完
│   ├── settings.py        「设置」中心：左侧导航（时间轴 / 干员与心情 / 换心情 / 闲置入宿）+ 接上 app 的落地回调
...
| **所有设置都在一个窗口里** | 工具栏只有一颗「**设置…**」，左边导航选分区：**时间轴**（周期 / 班数 / 每班时长 / 周期数）、**干员与心情**、**换心情**、**闲置入宿**。**改完立即生效**，关掉窗口即接受（不再有"点开→改→应用"三步）；工具栏右侧那行状态摘要（点它也能开设置）随时告诉你两个开关的状态 |
| 自己设置周期 / 班数 / 每班时长 / 周期数 | 「设置…」→ 时间轴：各班长之和必须等于周期，界面实时校验（不合法就不落地）；周期数 1~3 用来判断这套排班能不能永动 |
...
| **闲置入宿** | 「设置…」→ **闲置入宿**：把**没在上班、也不在宿舍、心情还没满**的干员安排进宿舍恢复——宿舍有空位就直接放进去（氛围高的优先），**没空位就与宿舍里心情已满的那位互换**（那位换出来闲置；他已是满心情，闲置不掉心情）。一个总开关 + **一张按时间排的逐次表**（第 1 周期第 1 班 → … → 第 N 周期最后一班，每组组头写时刻、自带全选/全不选；「去哪／与谁换」= **空位**（宿舍01、宿舍02…＝放进那间宿舍的空位，不动任何人）或 **满心情的人**（与他互换，他换出来闲置），只列**那一刻真的可选**的那些）。设置粒度是 **(周期, 班次, 干员)**，因为心情跨班跨周期连续、每次的候选都不一样；**改动立即生效**（防抖 250ms，表和主界面一起刷新）。默认**关闭**；曲线图上用绿色「宿」标记标出每次入宿 |
| **干员与心情（原「批量设置」）** | 「设置…」→ **干员与心情**：把**当前布局的所有干员 + 练度 + 心情**摊成一张可滚动表（房间 · 位次 · 干员 · **练度** · 心情）一次改完。心情区：全部 24 / 全部 0 / 统一设为 X / **按当前时刻回填** / **恢复导入值**（＝原来的工具栏「重置心情」）；干员区：**批量粘贴名单**（按房间顺序填入）/ 清空本班次 / 显示空位；**练度区：逐人下拉（E0/E1/E2）＋「全部设为 E2」**。点干员名可搜索更换，切班次下拉可逐班改。**改动立即生效**（心情输入防抖 250ms） |
...
结论：250 条全部有结论（242 条生效核对 + 8 条按性质归类），描述对照 **246 条一致 +
4 条无数字可对（消除/事件类），0 处差异**（`documents/05-技能分类大纲.md` §5.11）。
