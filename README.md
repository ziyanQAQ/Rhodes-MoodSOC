...
| **进驻事件（换心情）** | 工具栏是一颗「**换心情设置**」按钮 + 右侧**当前状态**（`未开启` / `已开启 · 换塞雷娅 · 等她满` / `已开启 · 按班次`）；**开关只有设置框里这一处**（旧版工具栏那个复选框已删掉，避免同一个开关出现两次）。框里是**四个设置**：① 开启心情交换、② 换谁（前一位进驻 / 全基建最累的 / 具体干员——**每个选项自带范围**）、③ 强制切换（勾＝她没满就**等她回满再换**；不勾＝判定时没满**就不换**）、④ **换哪个班**（班次多选、**默认全选**，可一次把同一套设置套到多个班；**没勾的班次不换心情**）。「对方心情是多少」不设开关（照换）；「位置也一起互换」收成固定口径（只换心情）；「某班想换给别人 / 想改 ③」收进**折叠的高级区**。状态栏另有一句完整口径（如 `换心情：未开启（…）`），应用后整句复述当前配置。场景 JSON 顶层的 `entry_events` 会在导入时自动同步到这里 |
...
**输入 → 输出**是否正确，不测试任何内部结构 / 内部函数。当前共 **173 个用例全绿**。
