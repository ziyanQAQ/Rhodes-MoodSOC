...
工作干员的基础消耗是 **1 点/时**（需求文档第 4 段），中枢满员再全局 -0.25，制造/贸易按进驻人数 -0.05/-0.1：

| 设施 | 每小时基础消耗 | 说明 |
|---|---|---|
| 制造站 / 贸易站 / 发电站 / 办公室 / 会客室 / 控制中枢 | 1.0 | 中枢另给全局减免 |
| **训练室** | **1.0** | 另有 9 条技能「心情每小时消耗 +1」→ 持有人 2.0/h |
| **加工站** | **0** | 心情按**配方次**扣，不搓材料就是 0（见下） |
| 宿舍 / 活动室 | 0 | 活动室「不视作入住在基建内」 |

训练室（持有人会比常人快一倍红脸）：

```
W「索然无味」= 1.0（设施）+ 1.0（自身技能）= 2.0/h  → 一管 24 点心情只能连续协助 12h
同设施的路人                              = 1.0/h
```

9 条来源（上游 `buffs`，原文均为「…时，心情每小时消耗+1」）：工作狂 / 过量训练 / 索然无味 /
何须解脱 / 变异 / 斗争渴望 / 与人乐 / 兴之所至·β / “手段应当有效”。

加工站：上游 35 条加工站心情 buff **全部**说的是「**配方**心情消耗」
（「心情消耗为 4 的配方全部 -1」「相应配方的心情消耗恒定为 2」「全部除以 4」…），
即心情是按次扣的。但**配方自身的心情消耗在上游数据 dump 里没有字段**，
所以"一次加工扣多少心情"暂不建模（棘刺「爆炸艺术」同理）。
