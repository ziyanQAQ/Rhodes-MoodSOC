...
| **改房间等级（容量随之变）** | 点看板**房间卡头** → 选 Lv（括号里写「可放 N 人」）；不足时卡头 `人数/容量` 变红。批量设置里也有「房间等级」区 |
...
| **批量设置** | 工具栏「**批量设置…**」：把**当前布局的所有干员 + 练度 + 心情**摊成一张可滚动表（房间 · 位次 · 干员 · **练度** · 心情）一次改完。心情区：全部 24 / 全部 0 / 统一设为 X / **按当前时刻回填** / 恢复导入值；干员区：**批量粘贴名单**（按房间顺序填入）/ 清空本班次 / 显示空位；**练度区：逐人下拉（E0/E1/E2）＋「全部设为 E2」**。点干员名可搜索更换，切班次下拉可逐班改 |
